...
abstract class nclspdo{
 ...
 /**
    ...
    private function loadSchema($strTblName) {
        if (!$this->getFromCache($strTblName)) {
            ...
            foreach ($schema as $value) {
                ...
                $this->schema[$strTblName][$value['Field']]['allownull'] = $value['Null'];
                $this->schema[$strTblName][$value['Field']]['default'] = $value['Default'];
            }
        ...
        foreach($data as $valData => $value){
            if($value){
                foreach($value as $valSubKey => $val){
                    if(trim($val) !== "") {
                        $arrData[$valData][$valSubKey] = $val;
                    }else{
                        if(!array_key_exists($valData, $this->schema)) $this->loadSchema($valData);
                        if(array_key_exists($valSubKey, $this->schema[$valData])){
                           if($this->schema[$valData][$valSubKey]['allownull'] == 'YES'){
                              $arrData[$valData][$valSubKey] = $this->getNull();
                           }elseif($this->schema[$valData][$valSubKey]['allownull'] == 'NO'){
                                $arrData[$valData][$valSubKey] = $this->schema[$valData][$valSubKey]['default'];
                            }else{
                                $arrData[$valData][$valSubKey] = $this->getDefaultFieldValue($this->schema[$valData][$valSubKey]['type']);
                            }
                        }
                    }
                }
	...
	/**
     * Function to get the default value in the case in which the field doesn't
	 * allow null & default value not set
     * @param String $dataType
     */
    private function getDefaultFieldValue($dataType){

        $arrIntDataTypes = array('int','tinyint','smallint','real','money','float','bit','decimal','numeric','smallmoney','bigint','mediumint','double','bool','serial');
        $arrCharDataTypes = array('text','ntext','char','nvarchar','nchar','xml','varbinary','binary','varchar','tinytext','blob','mediumtext','mediumblob','longtext','longblob');
        $arrDateDataTypes = array('date','datetime','timestamp','time','datetime2','datetimeoffset','smalldatetime','year');

        if(in_array($dataType, $arrIntDataTypes)) $defaultFieldValue = 0;
        if(in_array($dataType, $arrCharDataTypes)) $defaultFieldValue = '';
        if(in_array($dataType, $arrDateDataTypes)) $defaultFieldValue = $this->getNull();
    ...
    protected function del($tableName, $data, $params=null) {
        ...
        if(is_array($params) && array_key_exists('condition', $params)){
            ...
            if(isset($data[$tableName]['id'])) $sql = "delete from {$tableName} where id = {$data[$tableName]['id']};";
            if(isset($data[$tableName]['ID'])) $sql = "delete from {$tableName} where ID = {$data[$tableName]['ID']};";
        }
		...
		foreach ($data[$tableName] as $field => $value) {
			if (in_array($this->schema[$tableName][$field]['type'], $this->arrDataTypes)) {
                 if (strtoupper($value) === 'NULL' || $value === NULL) {
                    $data[$tableName][$field] = $value;
                } else {
                    if (in_array($this->schema[$tableName][$field]['type'], array('date', 'datetime', 'smalldatetime')))
                        $value = $this->changeDateFormat($value, $this->schema[$tableName][$field]['type']);
  ...
  /**
	...
	protected  function get($tableName,$params = null) {
		...
		$fields =  array();
//		pr($params);
		$condition = $order = $limit = $group = $having = $join = $offset = $fieldsArr = $sql = $top =   "";
		...
		if($params != null){
			...
			if(array_key_exists('limit', $params)){
					...
					if(count($params['limit'])>1){
						...
						$limit = (!$isMicrosoft)?$params['limit'][1]:$offset + $params['limit'][1] - 1;
					}
					elseif(!$isMicrosoft){
						$offset = 0;
						$limit = (is_array($params['limit']))?$params['limit'][0]:$params['limit'];
					}
					else{
						$top = " TOP ";
						$top .= (is_array($params['limit']))?$params['limit'][0]:$params['limit'];
					}
...
//		pr($sql);
//		pr("SELECT {$top} ".implode(', ', $fields)." FROM {$tableName} {$join} {$condition} {$group} {$having} {$order} {$limit}");
		$result = $this->query($sql);
		if(!$result){
			pr($sql);
			return false;
	...
	private function addSlashesDeep($value) {
	    ...
	    /**
     * Function to remove null values while selecting
     * @param <type> $value
     * @return <type>
     */
    private function removeNulls($value) {
        $value = is_array($value) ? array_map(array($this, 'removeNulls'), $value) : $this->removeNullValue($value);
        return $value;
    }

    /**
     * Function to remove the null values
     * @param string $value
     * @return string
     */
    private function removeNullValue($value) {
                ...
                if(count($conditions) > 0 && count($schema) > 0){
                    foreach ($conditions as $key => $value) {
                            ...
                            switch (true) {
                                    ...
                                    default:
										if($strTmpTableName != "") {
											if(!(strpos($strTmpTableName, '(') !== FALSE && ($schema[$key1]['type'] == 'date' || $schema[$key1]['type'] == 'datetime'))){
												$value = $this->value($value, $schema[$key1]);
											}
										}
...
//		pr($strJoinQuery);
    ...
    private function fetchValues(&$result) {
        ...
        $records = $result->fetchAll();
		      $records = $this->removeNulls($records);
        return $records;
    }
